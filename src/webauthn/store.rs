...
pub mod as_string {
	...
	pub fn serialize<S: serde::Serializer>(passkey: &Passkey, serializer: S) -> Result<S::Ok, S::Error> {
		use serde::ser::Error;
		let json = serde_json::to_string(passkey).map_err(Error::custom)?;
		serializer.serialize_str(&json)
	}

	pub fn deserialize<'de, D: serde::Deserializer<'de>>(deserializer: D) -> Result<Passkey, D::Error> {
		use serde::de::Error;
