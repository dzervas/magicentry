...
#[axum::debug_handler]
async fn oidc_authcode_handler(State(state): State<AppState>) -> impl IntoResponse {
	sqlx::query_scalar!(
		"SELECT code FROM user_secrets WHERE code LIKE 'me_oa_%' ORDER BY rowid DESC LIMIT 1"
	)
	.fetch_optional(&state.db)
	.await
	.unwrap()
	...
	let (addr, server) = axum_run(
		...
		Some(|router| {
			router
				...
				.route("/oidc-authcode", get(oidc_authcode_handler))
