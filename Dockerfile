...
RUN test "$TARGETPLATFORM" = "linux/arm64" || exit 0 && apt-get update && apt-get install --no-install-recommends -y gcc-aarch64-linux-gnu libc6-dev-arm64-cross && apt-get clean
